...
def get_heatmap_data_logrithmic(
    ...
    def calculate_logbase(num_buckets, max_number):
        ...
        return base 
    base = calculate_logbase(x_bucket_count, max(pauses_ms))
    ...
    for pause, time in zip(pauses_ms, times_seconds):
        if pause < base:
            ...
            bucket_no = int(math.log(pause, base))
...
##  C code
    ...
    self.unit_magnitude = int(math.floor(math.log(lowest_trackable_value) / math.log(2))) # int min
    largest_value_single_unit_res = 2 * math.pow(10, significant_figures)
    subb_count_mag = int(math.ceil(math.log(largest_value_single_unit_res) / math.log(2))) # 3.52
    self.sub_bucket_half_count_magnitude = subb_count_mag - 1 if subb_count_mag > 1 else 0
    self.sub_bucket_count = int(math.pow(2, self.sub_bucket_half_count_magnitude + 1)) #13
