...
def getConcurrentDurations(create_csv=False):
    ...
    table = g1f.manyMatch_LineSearch(
        match_terms=match_terms, num_match_groups=num_groups, data=[], filepath=path, in_file=True
    )
...
def getGCdataSections(create_csv=False):
    ...
    table = g1f.manyMatch_LineSearch(
        match_terms=[search_term], num_match_groups=6, data=[], filepath=path, in_file=True
    )
